...
def frost(x, severity=1):
    ...
    frost = cv2.imread(os.path.join('make_corruptions', filename))
...
def save_distorted_for_image(method, candi_image_names, severity, data_path, save_path):
    ...
    for frame in range(10):
        ...
        distorted_dataset_loader = torch.utils.data.DataLoader(
            distorted_dataset, batch_size=128, shuffle=False, num_workers=16)
...
parser.add_argument('--corruption', type=str, default='gaussian_noise', choices=['all', 'gaussian_noise', 'shot_noise', 'impulse_noise', 'defocus_blur', 'glass_blur', 'motion_blur', 'zoom_blur', 'snow', 'frost', 'fog', 'brightness', 'contrast', 'elastic_transform', 'pixelate', 'jpeg_compression'], help='Type of corruption to apply')
...
if args.corruption == 'all':
    ...
    d['Gaussian Noise'] = gaussian_noise
elif args.corruption == 'shot_noise':
    d['Shot Noise'] = shot_noise
elif args.corruption == 'impulse_noise':
    d['Impulse Noise'] = impulse_noise
elif args.corruption == 'defocus_blur':
    d['Defocus Blur'] = defocus_blur
elif args.corruption == 'glass_blur':
    d['Glass Blur'] = glass_blur
elif args.corruption == 'motion_blur':
    d['Motion Blur'] = motion_blur
elif args.corruption == 'zoom_blur':
    d['Zoom Blur'] = zoom_blur
    ...
    d['Brightness'] = brightness
elif args.corruption == 'contrast':
    d['Contrast'] = contrast
elif args.corruption == 'elastic_transform':
    d['Elastic'] = elastic_transform
